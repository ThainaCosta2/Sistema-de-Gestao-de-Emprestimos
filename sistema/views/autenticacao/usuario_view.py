...
def historico_pagamentos():
    ...
    if session.get('perfil') == 'cliente':
        ...
        for periodo in movimentacoes_cliente:
            ...
            periodo['valor_fixo_formatado'] = f"R$ {valor_parcela_float:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
            periodo['acrescimos_formatado'] = "R$ 0,00"
            periodo['valor_atualizado_formatado'] = periodo['valor_fixo_formatado']
            ...
            if periodo['status'] == 'Atrasado':
                ...
                if dias_atraso > 0:
                    ...
                    periodo['acrescimos_formatado'] = f"R$ {formatar_valor_moeda(acrescimos_float)}"
                    periodo['valor_atualizado_formatado'] = f"R$ {formatar_valor_moeda(valor_atualizado_float)}"
        ...
        if cliente_atual and movimentacoes_cliente:
            ...
            resumo_pagamentos['total_emprestimo'] = f"R$ {total_emprestimo_str}"
