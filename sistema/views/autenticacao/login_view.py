...
def principal():
    ...
    for cliente in clientes_completos_para_gerente:
        ...
        dias_atraso = 0
        if cliente['detalhes'][0].get('status') != 'Pago' and data_vencimento < hoje_data:
            dias_atraso = (hoje_data - data_vencimento).days

        valor_vencido_formatado = cliente['valor_parcela']
        cliente_status_gerente = 'Em Dia'
        if dias_atraso > 0:
            cliente_status_gerente = 'Atrasado'
            qtd_atrasados_geral += 1

            # Calcula o valor atualizado (Base + Multa + Juros) para exibição na tabela de pendências
            multa = valor_parcela_original * multa_percentual_gerente
            juros_calc = (valor_parcela_original * juros_mora_diario_percentual_gerente) * dias_atraso
            juros_final = min(juros_calc, valor_parcela_original * juros_max_percentual_gerente)
            valor_vencido_formatado = formatar_valor_moeda(valor_parcela_original + multa + juros_final)
        else:
        ...
        info_cliente = {
            ...
            'valor_vencido': valor_vencido_formatado,
